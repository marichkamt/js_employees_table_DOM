...
const sortTable = (index)=>{
    ...
    if (lastClickedIndex === index) sortDirection = sortDirection === "ASC" ? "DESC" : "ASC";
    ...
    rows.sort((a, b)=>{
        ...
        if (!isNaN(valueA) && !isNaN(valueB)) return sortDirection === "ASC" ? valueA - valueB : valueB - valueA;
        else return sortDirection === "ASC" ? textA.localeCompare(textB) : textB.localeCompare(textA);
    });
    rows.forEach((row)=>{
...
const rowSelection = (e)=>{
    ...
    if (clickedElement.classList.contains("active")) clickedElement.classList.remove("active");
    else {
...
fields.forEach((field)=>{
    form.appendChild(createFormElement(field.label, field.name, field.type, field.qa, field.options));
});
...
document.querySelector("form").addEventListener("submit", function(e) {
    ...
    if (parseInt(formData.get("age"), 10) > 90 || parseInt(formData.get("age"), 10) < 18) {
        isValid = false;
        errorMessage = "Age must be between 18 and 90.";
    }
    if (!isValid) pushNotification("Error", errorMessage, "error");
    else {
        const employee = {
        ...
        for(const key in employee)if (employee.hasOwnProperty(key)) {
